...
import org.creativeturbogiants.crypto.java.ge_p2;
import org.creativeturbogiants.crypto.java.ge_p3;

import static org.creativeturbogiants.crypto.ed25519.gen_labelset.labelset_new;
import static org.creativeturbogiants.crypto.java.ge_double_scalarmult.ge_double_scalarmult_vartime;
import static org.creativeturbogiants.crypto.ed25519.gen_labelset.*;
import static org.creativeturbogiants.crypto.java.ge_frombytes.ge_frombytes_negate_vartime;
import static org.creativeturbogiants.crypto.java.ge_p3_tobytes.ge_p3_tobytes;
import static org.creativeturbogiants.crypto.java.ge_scalarmult_base.ge_scalarmult_base;
import static org.creativeturbogiants.crypto.java.ge_tobytes.ge_tobytes;
import static org.creativeturbogiants.crypto.java.sc_muladd.sc_muladd;
import static org.creativeturbogiants.crypto.java.sc_reduce.sc_reduce;
...
public class gen_eddsa {
    ...
    */
                           ...
                           byte[] M_buf, long M_start, long M_len)
    {
        ...
        if (constants.HASHLEN != 64) {
            zeroize_commit(hash, M_buf, M_start, prefix_len);
            return -1;
        }

        prefix_len = 0;
        prefix_len += constants.POINTLEN + labelset_len + constants.RANDLEN;
        prefix_len += ((constants.BLOCKLEN - (prefix_len % constants.BLOCKLEN)) % constants.BLOCKLEN);
        prefix_len += constants.SCALARLEN;
        prefix_len += ((constants.BLOCKLEN - (prefix_len % constants.BLOCKLEN)) % constants.BLOCKLEN);
        prefix_len += labelset_len + constants.POINTLEN + extra_len;
        ...
        bufptr = bufstart;
        bufend = M_start;
        bufptr = buffer_add(B_bytes, 0, constants.POINTLEN, M_buf, bufptr, bufend);
        bufptr = buffer_add(labelset, 0, labelset_len, M_buf, bufptr, bufend);
        bufptr = buffer_add(Z, 0, constants.RANDLEN, M_buf, bufptr, bufend);
        bufptr = buffer_pad(M_buf, bufptr, bufend, bufstart);
        bufptr = buffer_add(k_scalar, 0, constants.SCALARLEN, M_buf, bufptr, bufend);
        bufptr = buffer_pad(M_buf, bufptr, bufend, bufstart);
        bufptr = buffer_add(labelset, 0, labelset_len, M_buf, bufptr, bufend);
        bufptr = buffer_add(K_bytes, 0, constants.POINTLEN, M_buf, bufptr, bufend);
        ...
        System.arraycopy(hash, 0, r_scalar, 0, (int) constants.SCALARLEN);
    ...
    private static void zeroize_commit(byte[] hash, byte[] M_buf, long M_start, long prefix_len){
        byte[] zero = new byte[(int) constants.HASHLEN];
        System.arraycopy(zero, 0, hash, 0, (int) constants.HASHLEN);
    ...
    */
                              ...
                              Sha512 sha512provider)
    {
        ...
        if (h_scalar.length != constants.SCALARLEN)
            return -1;
        ...
        if (labelset_is_empty(labelset, labelset_len)) {
            if (2* constants.POINTLEN > M_start)
              return -1;
            if (extra != null || extra_len != 0)
              return -1;
            System.arraycopy(R_bytes, 0, M_buf, (int) (M_start - (2* constants.POINTLEN)), (int) constants.POINTLEN);
            System.arraycopy(K_bytes, 0, M_buf, (int) (M_start - (1* constants.POINTLEN)), (int) constants.POINTLEN);
            prefix_len = 2* constants.POINTLEN;
        } else {
            prefix_len = 3* constants.POINTLEN + 2*labelset_len + extra_len;
            if (prefix_len > M_start)
              return -1;
            ...
            bufptr = buffer_add(B_bytes, 0, constants.POINTLEN, M_buf, bufptr, bufend);
            bufptr = buffer_add(labelset, 0, labelset_len, M_buf, bufptr, bufend);
            bufptr = buffer_add(R_bytes, 0, constants.POINTLEN, M_buf, bufptr, bufend);
            bufptr = buffer_add(labelset, 0, labelset_len, M_buf, bufptr, bufend);
            bufptr = buffer_add(K_bytes, 0, constants.POINTLEN, M_buf, bufptr, bufend);
            bufptr = buffer_add(extra, 0, extra_len, M_buf, bufptr, bufend);
        ...
        sc_reduce(hash);
        System.arraycopy(hash, 0, h_scalar, 0, (int) constants.SCALARLEN);
        return 0;
    }
                  ...
                  byte[] customization_label, long customization_label_len)
    {
        ...
        long labelset_len = 0;
        byte[] R_bytes = new byte[constants.POINTLEN];
        byte[] r_scalar = new byte[constants.SCALARLEN];
        ...
        if (signature_out.length != constants.SIGNATURELEN){
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        ...
        if (labelset_new(labelset, labelset_len, constants.LABELSETMAXLEN, null, 0,
        ...
        if (generalized_commit(R_bytes, r_scalar, labelset, labelset_len, null, 0,
                ...
                random, sha512provider, M_buf, constants.MSTART, msg_len) != 0) {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }

        if (generalized_challenge(h_scalar, labelset, labelset_len, null, 0,
                R_bytes, eddsa_25519_pubkey_bytes, M_buf, constants.MSTART, msg_len, sha512provider) != 0){
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        ...
        System.arraycopy(R_bytes, 0, signature_out, 0, constants.POINTLEN);
        System.arraycopy(s_scalar, 0, signature_out, constants.POINTLEN, constants.SCALARLEN);
    ...
    private static void zeroize_sign(byte[] r_scalar, byte[] M_buf){
        r_scalar = new byte[constants.SCALARLEN];
