...
import org.creativeturbogiants.crypto.java.crypto_verify_32;
import org.creativeturbogiants.crypto.java.ge_p3;

import static org.creativeturbogiants.crypto.ed25519.gen_labelset.labelset_validate;
import static org.creativeturbogiants.crypto.ed25519.gen_eddsa.*;
import static org.creativeturbogiants.crypto.ed25519.gen_labelset.*;
...
public class gen_veddsa {
                                               ...
                                               Sha512 sha512provider)
    {
        ...
        prefix_len = 2* constants.POINTLEN + labelset_len;
        ...
        M_buf_ptr = buffer_add(B_bytes, 0, constants.POINTLEN, M_buf, M_buf_ptr, M_start);
        M_buf_ptr = buffer_add(labelset, 0, labelset_len, M_buf, M_buf_ptr, M_start);
        M_buf_ptr = buffer_add(K_bytes, 0, constants.POINTLEN, M_buf, M_buf_ptr, M_start);
    ...
    static int generalized_calculate_vrf_output(byte[] vrf_output,
                                                ...
                                                Sha512 sha512provider)
    {
        if (vrf_output.length != constants.VRFOUTPUTLEN)
            return -1;

        byte[] buf = new byte[(int) constants.BUFLEN];
        long buflen = constants.BUFLEN;
        byte[] cKv_bytes = new byte[(int) constants.POINTLEN];
        byte[] hash = new byte[(int) constants.HASHLEN];

        if (vrf_output == null)
            return -1;

        if (labelset_len + 2* constants.POINTLEN > constants.BUFLEN)
            return -1;
        if (labelset_validate(labelset, labelset_len) != 0)
            return -1;
        if (cKv_point == null)
            return -1;
        if (constants.VRFOUTPUTLEN > constants.HASHLEN)
            return -1;

        ge_p3_tobytes.ge_p3_tobytes(cKv_bytes, cKv_point);
        long bufptr = 0;
        bufptr = buffer_add(B_bytes, 0, constants.POINTLEN, buf, bufptr, buflen);
        bufptr = buffer_add(labelset, 0, labelset_len, buf, bufptr, buflen);
        bufptr = buffer_add(cKv_bytes, 0, constants.POINTLEN, buf, bufptr, buflen);
        if (bufptr < 0)
            return -1;
        if (bufptr > constants.BUFLEN)
            return -1;
        sha512provider.calculateDigest(hash, buf, bufptr);
        System.arraycopy(hash, 0, vrf_output, 0, (int) constants.VRFOUTPUTLEN);
    ...
    {
        byte[] labelset = new byte[(int) constants.LABELSETMAXLEN];
        long labelset_len = 0;
        ge_p3 Bv_point = new ge_p3();
        ge_p3 Kv_point = new ge_p3();
        ge_p3 Rv_point = new ge_p3();
        byte[] Bv_bytes = new byte[(int) constants.POINTLEN];
        byte[] Kv_bytes = new byte[(int) constants.POINTLEN];
        byte[] Rv_bytes = new byte[(int) constants.POINTLEN];
        byte[] R_bytes = new byte[(int) constants.POINTLEN];
        byte[] r_scalar = new byte[(int) constants.SCALARLEN];
        byte[] h_scalar = new byte[(int) constants.SCALARLEN];
        byte[] s_scalar = new byte[(int) constants.SCALARLEN];
        byte[] extra = new byte[(int) (3* constants.POINTLEN)];
        byte[] M_buf = new byte[(int) (msg_len + constants.MSTART)];
        byte[] protocol_name = "VEdDSA_25519_SHA512_Elligator2".getBytes();

        if (signature_out == null || signature_out.length != constants.VRFSIGNATURELEN) {
        ...
        if (customization_label_len > constants.LABELMAXLEN) {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        if (msg_len > constants.MSGMAXLEN) {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }

        System.arraycopy(msg, 0, M_buf, (int) constants.MSTART, (int) msg_len);

        labelset_len = labelset_new(labelset, labelset_len, constants.LABELMAXLEN,
        ...
        labelset_len = labelset_add(labelset, labelset_len, constants.LABELSETMAXLEN, "1".getBytes(), 1);
        if (labelset_len < 0) {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        ...
        if (generalized_calculate_Bv(Bv_point, labelset, labelset_len,
            eddsa_25519_pubkey_bytes, M_buf, constants.MSTART, msg_len, sha512provider) != 0)
        {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        ...
        System.arraycopy(Bv_bytes, 0, extra, 0, (int) constants.POINTLEN);
        System.arraycopy(Kv_bytes, 0, extra, (int) constants.POINTLEN, (int) constants.POINTLEN);

        if (generalized_commit(R_bytes, r_scalar,
                                         labelset, labelset_len,
                                         extra, 2* constants.POINTLEN,
                                         eddsa_25519_pubkey_bytes, eddsa_25519_privkey_scalar,
                                         random, sha512provider,
                                         M_buf, constants.MSTART, msg_len) != 0)
        {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        }
        ...
        System.arraycopy(Rv_bytes, 0, extra, (int)(2* constants.POINTLEN), (int) (constants.POINTLEN));
        if (generalized_challenge(h_scalar,
                                  labelset, labelset_len,
                                  extra, 3* constants.POINTLEN,
                                  R_bytes, eddsa_25519_pubkey_bytes,
                                  M_buf, constants.MSTART, msg_len,
                                  sha512provider) != 0)
        {
            zeroize_sign(r_scalar, M_buf);
            return -1;
        ...
        //  return (Kv || h || s)
        System.arraycopy(Kv_bytes, 0, signature_out, 0, (int) constants.POINTLEN);
        System.arraycopy(h_scalar, 0, signature_out, (int) constants.POINTLEN, (int) constants.SCALARLEN);
        System.arraycopy(s_scalar, 0, signature_out, constants.POINTLEN+ constants.SCALARLEN, constants.SCALARLEN);
    ...
    private static void zeroize_sign(byte[] r_scalar, byte[] M_buf){
        byte[] zero = new byte[constants.SCALARLEN]; //ZEROIZE r_scalar
        System.arraycopy(zero, 0, r_scalar, 0, constants.SCALARLEN);
    ...
    static int generalized_veddsa_25519_verify(byte[] vrf_out,
                  ...
                  Sha512 sha512provider)
    {
        byte[] labelset = new byte[constants.LABELSETMAXLEN];
        long labelset_len = 0;
        byte[] Kv_bytes = new byte[constants.POINTLEN];
        byte[] h_scalar = new byte[constants.SCALARLEN];
        byte[] s_scalar = new byte[constants.SCALARLEN];
        ge_p3 Bv_point = new ge_p3(), K_point = new ge_p3(), Kv_point = new ge_p3(), cK_point = new ge_p3(), cKv_point = new ge_p3();
        byte[] Bv_bytes = new byte[constants.POINTLEN];
        byte[] R_calc_bytes = new byte[constants.POINTLEN];
        byte[] Rv_calc_bytes = new byte[constants.POINTLEN];
        byte[] h_calc_scalar = new byte[constants.SCALARLEN];
        byte[] extra = new byte[3* constants.POINTLEN];
        byte[] M_buf = new byte[(int) (msg_len + constants.MSTART)];
        byte[] protocol_name = "VEdDSA_25519_SHA512_Elligator2".getBytes();
        ...
        if (vrf_out.length != constants.VRFOUTPUTLEN){
            zeroize_verify(M_buf);
            return -1;
        }
        if (signature == null){
        ...
        System.arraycopy(msg, 0, M_buf, constants.MSTART, (int) msg_len);

        System.arraycopy(signature, 0, Kv_bytes, 0, constants.POINTLEN);
        System.arraycopy(signature, constants.POINTLEN, h_scalar, 0, constants.SCALARLEN);
        System.arraycopy(signature, constants.POINTLEN+ constants.SCALARLEN, s_scalar, 0, constants.SCALARLEN);
        ...
        labelset_len = labelset_new(labelset, labelset_len, constants.LABELSETMAXLEN,
                protocol_name, protocol_name.length,
                customization_label, customization_label_len);
        if (labelset_len < 0) {
            zeroize_verify(M_buf);
        ...
        System.arraycopy(Bv_bytes, 0, extra, 0, constants.POINTLEN);
        System.arraycopy(Kv_bytes, 0, extra, constants.POINTLEN, constants.POINTLEN);
        System.arraycopy(Rv_calc_bytes, 0, extra, 2* constants.POINTLEN, constants.POINTLEN);
