...
public class gen_labelset {
    ...
    static long buffer_pad(byte[] buf, long pos, long len, long correction)
    {
        ...
        pad_len = (constants.BLOCKLEN - ((pos-correction) % constants.BLOCKLEN)) % constants.BLOCKLEN;
        if (len - pos < pad_len)
            return -1;
                             ...
                             byte[] customization_label, long customization_label_len)
    {
        ...
        if (labelset == null)
            return -1;
        if (labelset_maxlen > constants.LABELSETMAXLEN)
            return -1;
        ...
        if (customization_label == null && customization_label_len > 0)
            return -1;
        if (protocol_name_len > constants.LABELMAXLEN)
            return -1;
                             ...
                             byte[] label, long label_len)
    {
        if (labelset_len < 0)
            return -1;
        if (labelset_len > constants.LABELSETMAXLEN || labelset_maxlen > constants.LABELSETMAXLEN)
            return -1;
        if (labelset_len >= labelset_maxlen || labelset_len + label_len + 1 > labelset_maxlen)
            return -1;
        if (labelset_len < 3 || labelset_maxlen < 4)
            return -1;
        if (label_len > constants.LABELMAXLEN)
            return -1;
    ...
    static int labelset_validate(byte[] labelset, long labelset_len)
    {
        ...
        if (labelset == null)
            return -1;
        if (labelset_len < 3 || labelset_len > constants.LABELSETMAXLEN)
            return -1;
